...
const DEFINITION_URL = "https://raw.githubusercontent.com/awslabs/diagram-as-code/main/definitions/definition-for-aws-icons-light.yaml";
...
async function main() {
    ...
    const pptxEntry = mainEntries.find((entry: AdmZip.IZipEntry) => entry.entryName === pptxName);
    if (!pptxEntry) {
         // Fallback: try to find any pptx
         const anyPptx = mainEntries.find((entry: AdmZip.IZipEntry) => entry.entryName.endsWith('.pptx'));
         if (anyPptx) {
             console.warn(`Warning: Could not find exact PPTX '${pptxName}', using '${anyPptx.entryName}'`);
    ...
    await fs.writeJson(path.join(MAPPINGS_DIR, "icon-map.json"), simpleMap, { spaces: 2 });
